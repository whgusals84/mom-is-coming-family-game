...
function drawFurniture(ctx: CanvasRenderingContext2D, item: (typeof FURNITURE)[number]) {
  const palettes: Record<string, [string, string]> = {
    sofa: ['#ea806b', '#a9453f'], tv: ['#444853', '#25272d'], table: ['#b47d4d', '#7d4f31'], plant: ['#79aa67', '#b85b4b'],
    fridge: ['#e9f3f2', '#96aaa9'], dining: ['#d39a5f', '#8f5c35'], bed: ['#98bfe8', '#587ea8'], desk: ['#ba8151', '#784b2f'],
    closet: ['#8e6bc0', '#60448c'], cabinet: ['#d9ad72', '#9a6a3e'], dollShelf: ['#f2b9c5', '#a85b72'],
    counter: ['#f1e4c5', '#9a8064'], toilet: ['#f7fbfb', '#93aaa9'], sink: ['#d8efed', '#77a6a0'], tub: ['#d2e9ef', '#6a9aaa'],
  };
  const [fill, edge] = palettes[item.kind ?? ''] ?? ['#c99462', '#805836'];
  ctx.save();
  ctx.shadowColor = 'rgba(63,44,34,.22)'; ctx.shadowBlur = 10; ctx.shadowOffsetY = 7;
  roundedRect(ctx, item.x, item.y, item.w, item.h, Math.min(18, item.h / 4));
  ctx.fillStyle = fill; ctx.fill();
  ctx.shadowColor = 'transparent'; ctx.lineWidth = 5; ctx.strokeStyle = edge; ctx.stroke();
  if (item.kind === 'tv') {
    roundedRect(ctx, item.x + 14, item.y + 10, item.w - 28, item.h - 20, 8); ctx.fillStyle = '#87d4dd'; ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,.65)'; ctx.fillRect(item.x + 28, item.y + 17, 34, 6);
  } else if (item.kind === 'sofa') {
    ctx.strokeStyle = edge; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(item.x + item.w / 2, item.y + 8); ctx.lineTo(item.x + item.w / 2, item.y + item.h - 8); ctx.stroke();
  } else if (item.kind === 'fridge') {
    ctx.fillStyle = edge; ctx.fillRect(item.x + item.w - 22, item.y + 18, 6, item.h - 36);
  } else if (item.kind === 'plant') {
    ctx.fillStyle = '#a65b43'; roundedRect(ctx, item.x + 10, item.y + 32, item.w - 20, item.h - 28, 10); ctx.fill();
    ...
    ctx.strokeStyle = edge; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(item.x + item.w / 2, item.y + item.h / 2, 16, 0, Math.PI * 2); ctx.stroke();
...
function drawPlush(ctx: CanvasRenderingContext2D, plush: (typeof PLUSHIES)[number]) {
  const s = plush.scale;
  ctx.save(); ctx.translate(plush.x, plush.y); ctx.scale(s, s);
  ctx.strokeStyle = '#59483f'; ctx.lineWidth = 2.5; ctx.fillStyle = plush.color;
  if (plush.ears === 'rabbit') {
    ctx.beginPath(); ctx.ellipse(-7, -18, 6, 15, -.18, 0, Math.PI * 2); ctx.ellipse(7, -18, 6, 15, .18, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  } else if (plush.ears === 'cat') {
    ctx.beginPath(); ctx.moveTo(-18, -12); ctx.lineTo(-12, -29); ctx.lineTo(-2, -14); ctx.moveTo(18, -12); ctx.lineTo(12, -29); ctx.lineTo(2, -14); ctx.fill(); ctx.stroke();
  } else {
    ctx.beginPath(); ctx.arc(-13, -13, 8, 0, Math.PI * 2); ctx.arc(13, -13, 8, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  }
  ctx.beginPath(); ctx.ellipse(0, 0, 22, 20, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#4a3932'; ctx.beginPath(); ctx.arc(-7, -3, 2.2, 0, Math.PI * 2); ctx.arc(7, -3, 2.2, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = plush.accent; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 4, 6, .2, Math.PI - .2); ctx.stroke();
  ctx.fillStyle = plush.accent; roundedRect(ctx, -16, 16, 32, 8, 4); ctx.fill(); ctx.restore();
}

function drawDoorMat(ctx: CanvasRenderingContext2D, x: number, y: number) {
  roundedRect(ctx, x - 75, y - 28, 150, 56, 15); ctx.fillStyle = '#65b9a8'; ctx.fill();
  ctx.strokeStyle = '#2d786c'; ctx.lineWidth = 4; ctx.stroke();
  ctx.fillStyle = '#fff7df'; ctx.font = '900 17px system-ui'; ctx.textAlign = 'center'; ctx.fillText('어서 와!', x, y + 6);
}
...
function drawFallbackCharacter(ctx: CanvasRenderingContext2D, role: CharacterRole, moving: boolean, time: number, mood: MomMood) {
  const tall = role === 'brother';
  const angry = role === 'mom' && (mood === 'chase' || mood === 'extreme');
  const bodyColor = role === 'player' ? '#47b5ad' : role === 'mom' ? '#f37d6e' : role === 'brother' ? '#5d6fab' : '#d9b266';
  const hair = role === 'mom' ? '#5b352c' : '#342821';
  const leg = moving ? Math.sin(time * 14) * 7 : 0;
  ctx.lineCap = 'round'; ctx.strokeStyle = '#342820'; ctx.lineWidth = 7;
  ctx.beginPath(); ctx.moveTo(-8, 3); ctx.lineTo(-10 + leg, 20); ctx.moveTo(8, 3); ctx.lineTo(10 - leg, 20); ctx.stroke();
  roundedRect(ctx, -19, tall ? -43 : -37, 38, tall ? 50 : 44, 14); ctx.fillStyle = bodyColor; ctx.fill(); ctx.strokeStyle = '#342820'; ctx.lineWidth = 4; ctx.stroke();
  if (role === 'mom') {
    ctx.fillStyle = hair; ctx.beginPath(); ctx.ellipse(0, -57, 31, 38, 0, 0, Math.PI * 2); ctx.fill();
    if (angry) { ctx.beginPath(); ctx.ellipse(-24 + Math.sin(time * 18) * 5, -45, 13, 30, -.5, 0, Math.PI * 2); ctx.fill(); }
  }
  ctx.fillStyle = '#ffd5bd'; ctx.beginPath(); ctx.arc(0, tall ? -58 : -53, 27, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = '#342820'; ctx.lineWidth = 4; ctx.stroke();
  if (role !== 'mom') { ctx.fillStyle = hair; ctx.beginPath(); ctx.arc(0, tall ? -64 : -59, 27, Math.PI, 0); ctx.fill(); }
  ctx.fillStyle = '#342820';
  if (angry) { ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(-15, -62); ctx.lineTo(-5, -58); ctx.moveTo(15, -62); ctx.lineTo(5, -58); ctx.stroke(); }
  ctx.beginPath(); ctx.arc(-9, tall ? -56 : -51, 3, 0, Math.PI * 2); ctx.arc(9, tall ? -56 : -51, 3, 0, Math.PI * 2); ctx.fill();
  if (role === 'dad') drawGlasses(ctx, tall ? -56 : -51);
  ctx.strokeStyle = '#8e4b43'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(0, tall ? -48 : -43, 6, 0, Math.PI); ctx.stroke();
  if (role === 'brother') drawRabbit(ctx, -29, -18);
}

function drawGlasses(ctx: CanvasRenderingContext2D, y: number) {
  ctx.strokeStyle = '#282629'; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.arc(-9, y, 8, 0, Math.PI * 2); ctx.arc(9, y, 8, 0, Math.PI * 2); ctx.moveTo(-1, y); ctx.lineTo(1, y); ctx.stroke();
}

function drawRabbit(ctx: CanvasRenderingContext2D, x: number, y: number) {
  ctx.save(); ctx.translate(x, y); ctx.fillStyle = '#fffdf4'; ctx.strokeStyle = '#604c42'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.ellipse(-5, -13, 4, 11, -.15, 0, Math.PI * 2); ctx.ellipse(5, -13, 4, 11, .15, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.beginPath(); ctx.arc(0, 0, 11, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#604c42'; ctx.beginPath(); ctx.arc(-4, -2, 1.5, 0, Math.PI * 2); ctx.arc(4, -2, 1.5, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#63b9a8'; ctx.fillRect(-10, 7, 20, 5); ctx.restore();
}

export function drawSpeech(ctx: CanvasRenderingContext2D, at: Point, text: string, color = '#fffdf4') {
  ctx.save(); ctx.font = '900 15px system-ui';
  const width = Math.min(230, ctx.measureText(text).width + 30);
  const x = at.x - width / 2; const y = at.y - 108;
  roundedRect(ctx, x, y, width, 38, 13); ctx.fillStyle = color; ctx.fill(); ctx.strokeStyle = '#3b2d27'; ctx.lineWidth = 3; ctx.stroke();
  ctx.beginPath(); ctx.moveTo(at.x - 7, y + 37); ctx.lineTo(at.x, y + 48); ctx.lineTo(at.x + 8, y + 36); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#382a24'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(text, at.x, y + 19, width - 16); ctx.restore();
}

export function drawMarker(ctx: CanvasRenderingContext2D, at: Point, label: string, ready: boolean, pulse: number) {
  ctx.save(); ctx.translate(at.x, at.y - 20 - Math.sin(pulse * 5) * 4); ctx.globalAlpha = ready ? 1 : .35;
  ctx.fillStyle = ready ? '#fff36d' : '#fffdf4'; ctx.strokeStyle = '#3b2d27'; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.arc(0, 0, 18, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#3b2d27'; ctx.font = '1000 18px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('E', 0, 1);
  ctx.font = '800 13px system-ui'; ctx.fillStyle = '#493831'; ctx.fillText(label, 0, 31); ctx.restore();
}
